...
func (manager *Manager) retryEndpointIdentityLabelsResolution(ctx context.Context, wg *sync.WaitGroup) {
	...
	go func() {
		...
		retryQueue := manager.endpointEventRetryQueue
		go func() {
			select {
			case <-ctx.Done():
				retryQueue.ShutDown()
		...
		for {
			...
			if manager.endpointEventIsPending(endpointID) {
				// if the endpoint event is still pending it means the
				// manager failed again to resolve the endpoint ID to a set
				// of labels, so add back the item to the queue
				manager.endpointEventRetryQueue.AddRateLimited(endpointID)
			} else {
				// otherwise just remove it
				manager.endpointEventRetryQueue.Forget(endpointID)
			}

			manager.endpointEventRetryQueue.Done(endpointID)
...
func (manager *Manager) OnUpdateEndpoint(endpoint *k8sTypes.CiliumEndpoint) {
	...
	// remove the ID from the workqueue in case an older revision of the
	// same CiliumEndpoint was already present
	...
	if manager.endpointEventIsPending(id) {
		manager.endpointEventRetryQueue.Add(id)
	}
}
