...
pipeline {
    ...
    environment {
        ...
        BASE_IMAGE="""${sh(
                returnStdout: true,
                script: 'if [ "${run_with_race_detection}" = "" ]; then echo -n "scratch"; else echo -n "quay.io/cilium/cilium-runtime:437ab79d330bb6c20f7dc23038ee229d8d034f91@sha256:b53015973d4f31e672704a85d741facadfa4cc9ec9e5fe94f1eb3673d8c78cd0"; fi'
            )}"""
    }
